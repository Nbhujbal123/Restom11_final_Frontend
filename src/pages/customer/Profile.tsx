...
const Profile: React.FC = () => {
  ...
  const fetchBills = async (email?: string, name?: string) => {
    ...
    if (!emailToUse && !nameToUse && !userIdToUse) return
    setLoadingBills(true)
    console.log('Profile: fetchBills called', { userEmail: emailToUse, userName: nameToUse, userId: userIdToUse })
    try {
      // Fetch bills filtered by siteCode
      const siteCode = localStorage.getItem('siteCode') || ''
      const response = await fetch(`${API_BASE_URL}/bills?status=PAID&limit=100`, {
        headers: {
          'x-site-code': siteCode
        }
      })
  ...
  const fetchBillsByEmail = async (email?: string, name?: string) => {
    ...
    if (!emailToUse && !nameToUse && !userIdToUse) return
    setLoadingBills(true)
    console.log('Profile: fetchBillsByEmail called', { userEmail: emailToUse, userName: nameToUse, userId: userIdToUse })
    try {
      // Fetch bills filtered by siteCode
      const siteCode = localStorage.getItem('siteCode') || ''
      const response = await fetch(`${API_BASE_URL}/bills?status=PAID&limit=100`, {
            ...
            <div className="card border-0 shadow-sm" style={{ borderRadius: '16px' }}>
              <div className="card-body p-3">
                <div className="d-flex justify-content-between align-items-center mb-3">
                  <h6 className="mb-0 fw-bold">Paid Bills</h6>
                  <button 
                ...
                <div className="alert alert-info py-2 px-3 mb-3" style={{ fontSize: '0.8rem' }}>
                  <small>Only bills marked as <strong>PAID</strong> by the restaurant are shown here.</small>
                </div>
                {loadingBills ? (
                  <div className="text-center py-4">
                    <div className="spinner-border text-warning" role="status">
