...
const Login: React.FC<LoginProps> = ({ onClose, onSwitchToSignup, onLogin }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Simulate API call - keep loading until menu page is shown
    setTimeout(() => {
      onLogin(email, password, siteCode)
      // Navigate to menu instead of profile
      navigate('/menu')
      // Clear auth loading after a short delay to allow menu to render
      setTimeout(() => {
  ...
  // Show full-screen loading overlay when logging in - persists until menu is shown
  if (isLoading) {
    return (
      <LoadingSpinner 
        fullScreen={true} 
        message="Taking you to menu..." 
        size="large" 
      />
    )
  }
