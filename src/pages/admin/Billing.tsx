...
const Billing: React.FC = () => {
  ...
  const [loadingBillingOrders, setLoadingBillingOrders] = useState(true);
  const [selectedOrders, setSelectedOrders] = useState<string[]>([]);
  const [restaurants, setRestaurants] = useState<{ siteCode: string; name: string }[]>([]);
  
  const billsPerPage = 10;

  // Check authentication and siteCode
  useEffect(() => {
    const isAuthenticated = localStorage.getItem('adminAuthenticated');
    if (!isAuthenticated) navigate('/admin');
    
    // Ensure siteCode is set
    const siteCode = localStorage.getItem('siteCode');
    if (!siteCode) {
      console.warn('No siteCode found in localStorage. Bills may not display correctly.');
    }
  }, [navigate]);

  // Auto-generate bills on mount - find completed orders and create bills
  useEffect(() => {
    const autoGenerateBills = async () => {
      try {
        const siteCode = localStorage.getItem('siteCode') || '';
        console.log('Auto-generating bills on page load for siteCode:', siteCode);
        
        const response = await fetch(`${API_BASE_URL}/orders/debug-generate-bills`, {
          method: 'POST',
        ...
        if (response.ok) {
          ...
          console.log('Auto bill generation result:', data);
          if (data.billsCreated > 0 || data.billsUpdated > 0) {
            toast.success(`${data.billsCreated} new bills created, ${data.billsUpdated} updated`);
          }
        }
      } catch (error) {
  ...
  // Fetch billing orders (completed but not billed)
  const fetchBillingOrders = async () => {
    setLoadingBillingOrders(true);
    try {
      const siteCode = localStorage.getItem('siteCode') || '';
      const response = await fetch(`${API_BASE_URL}/orders/billing-orders`, {
        headers: {
  ...
  useEffect(() => {
    const fetchBills = async () => {
      ...
      try {
        ...
        // If super admin, fetch all restaurants for filtering
        ...
        console.log('SiteCode being used:', siteCode);
  ...
  // Handle order selection for billing
  ...
  const handleSelectAll = () => {
    if (selectedOrders.length === billingOrders.length) {
      setSelectedOrders([]);
    } else {
      setSelectedOrders(billingOrders.map(o => o._id));
    }
  };
  ...
  const handleMarkAsBilled = async () => {
    ...
    try {
      const siteCode = localStorage.getItem('siteCode') || '';
      const response = await fetch(`${API_BASE_URL}/orders/mark-billed`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'x-site-code': siteCode
      ...
      if (response.ok) {
        const data = await response.json();
        toast.success(`${data.count} orders marked as billed`);
        setSelectedOrders([]);
        fetchBillingOrders(); // Refresh the list
      } else {
        toast.error('Failed to mark orders as billed');
      }
    } catch (error) {
      console.error('Error marking orders as billed:', error);
      toast.error('Error marking orders as billed');
    }
  };
  ...
  return (
    <AdminLayout title="Billing & Payments">
      <div className="container-fluid py-4" style={{ backgroundColor: '#f8f9fa', minHeight: '100vh', color: '#212529', marginTop: '50px' }}>
        ...
        {/* No siteCode warning */}
        {!localStorage.getItem('siteCode') ? (
          <div className="alert alert-danger" role="alert">
            <strong>ERROR:</strong> No site code found in localStorage!
            <br/>
            <small>Please log out and log in again through the admin portal.</small>
          </div>
        ) : (
          <div className="alert alert-success py-2 px-3 mb-4" role="alert" style={{ fontSize: '0.85rem' }}>
            <small>Debug: Using siteCode: <strong>{localStorage.getItem('siteCode')}</strong></small>
          </div>
        )}
        
        {/* Generate Bills Button - Manual trigger */}
        <div className="mb-4">
          <button 
            ...
            onClick={async () => {
              try {
                setLoading(true);
                const siteCode = localStorage.getItem('siteCode') || '';
                console.log('Manually generating bills for siteCode:', siteCode);
                ...
                if (response.ok) {
                  const data = await response.json();
                  console.log('Generate bills response:', data);
                  toast.success(`Generated ${data.billsCreated || 0} new bills, updated ${data.billsUpdated || 0} existing bills`);
                } else {
                  const errorText = await response.text();
                  console.error('Generate bills error:', errorText);
          ...
          >
            <FaFileInvoice className="me-2" /> Generate Bills Now
          </button>
          
          {/* DEBUG: Generate bills from ALL completed orders in database */}
          <button 
            className="btn btn-warning me-2"
            onClick={async () => {
              try {
                setLoading(true);
                toast.info('Generating bills from ALL completed orders in database...');
                ...
                if (response.ok) {
                  const data = await response.json();
                  console.log('Debug generate bills response:', data);
                  toast.success(`Found ${data.completedOrdersFound} completed orders. Created ${data.billsCreated} bills, updated ${data.billsUpdated} bills. ${data.alreadyBilled} already billed.`);
                  
                  // Refresh bills list
                  const billsResponse = await fetch(`${API_BASE_URL}/bills?page=1&limit=100`, {
                    headers: { 'x-site-code': localStorage.getItem('siteCode') || '' }
                  });
                  if (billsResponse.ok) {
                    const billsData = await billsResponse.json();
                    setBills(billsData.bills || billsData);
                  }
                  
                  // Refresh stats
                  const statsResponse = await fetch(`${API_BASE_URL}/bills/stats`, {
                    headers: { 'x-site-code': localStorage.getItem('siteCode') || '' }
                  });
                  if (statsResponse.ok) {
                    const statsData = await statsResponse.json();
                    setBillStats(statsData);
                  }
                } else {
                  const errorText = await response.text();
                  console.error('Debug generate bills error:', errorText);
                  toast.error('Failed to generate bills');
                }
              } catch (error) {
                console.error('Error generating debug bills:', error);
                toast.error('Error generating bills');
              } finally {
                setLoading(false);
              }
        ...
        <div className="alert alert-info mb-4" role="alert">
          ...
          <strong>How billing works:</strong> When an order is marked as "Completed" in the Dashboard, a bill is automatically created. 
          Use the "Generate Bills Now" button if bills don't appear. Filter by "Unpaid" to see pending payments.
        </div>
        ...
        <div className="card border-0 shadow-sm mb-4" style={{ borderRadius: '12px' }}>
          <div className="card-body">
            <div className="row align-items-end">
              ...
              <div className="col-md-2 mb-3">
                <label className="form-label small text-muted">Restaurant</label>
                <div className="input-group">
                  <span className="input-group-text" style={{ backgroundColor: '#fff', borderColor: '#dee2e6' }}>
                    <FaFilter />
                  </span>
                  <select 
                    ...
                    onChange={(e) => handleFilterChange('siteCode', e.target.value)}
                    style={{ borderColor: '#dee2e6' }}
                  >
                    <option value="">All Restaurants</option>
                    {restaurants.map(restaurant => (
                      <option key={restaurant.siteCode} value={restaurant.siteCode}>
                        {restaurant.name} ({restaurant.siteCode})
                      </option>
                    ))}
                  </select>
                </div>
              ...
              <div className="col-md-2 mb-3">
                <button 
                  ...
                  onClick={() => setFilters({ status: 'ALL', search: '', startDate: '', endDate: '', siteCode: '' })}
                >
                  Clear Filters
                </button>
              </div>
        ...
        {/* Bills List - Bills are automatically created when orders are marked as COMPLETED */}
            ...
            <div className="d-flex justify-content-between align-items-center p-3" style={{ borderBottom: '2px solid #dee2e6' }}>
              ...
              <div className="d-flex align-items-center gap-2">
                <button 
                  ...
                  onClick={async () => {
                    // Manually generate bills for completed orders
                    try {
                      const siteCode = localStorage.getItem('siteCode') || ''
                      const response = await fetch(`${API_BASE_URL}/orders/generate-bills`, {
                        method: 'GET',
                      ...
                      if (response.ok) {
                        const data = await response.json()
                        toast.success(`Generated ${data.billsCreated || 0} new bills`)
                        // Refresh bills list
                        const billsResponse = await fetch(`${API_BASE_URL}/bills?page=1&limit=100`, {
              ...
              {currentBills.map(bill => (
                <div key={bill._id} className="bill-card m-3 p-3" style={{
                  ...
                  <div className="d-flex justify-content-between align-items-start mb-2">
                    <div>
                      <span className="badge me-1" style={{ backgroundColor: '#6366F1', color: '#fff' }}>
                        {bill.siteCode || 'N/A'}
                      </span>
