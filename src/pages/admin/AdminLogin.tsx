...
const AdminLogin: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    setIsLoading(true)
    setAuthLoading(true) // Start global auth loading
    setError('')
    ...
    try {
      ...
      // Store siteCode - use response siteCode or fall back to entered siteCode
      const siteCodeToStore = res.data.user?.siteCode || credentials.siteCode
      if (siteCodeToStore) {
        localStorage.setItem('siteCode', siteCodeToStore.toUpperCase())
      }
      
      // Redirect to dashboard
      navigate('/admin/dashboard')
      // Clear auth loading after a short delay to allow page to render
      setTimeout(() => {
  ...
  // Show full-screen loading overlay when logging in
  if (isLoading) {
    return (
      <LoadingSpinner 
        fullScreen={true} 
